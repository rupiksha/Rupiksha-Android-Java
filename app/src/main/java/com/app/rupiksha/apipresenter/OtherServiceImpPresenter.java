...
public class OtherServiceImpPresenter extends BasePresenter<IOtherServiceImpView>
{
    public static int retryCount = 0;

    public void getbillerlist(Activity context,HashMap<String, String> headers,String type,boolean isprocess) {
        // getView().enableLoadingBar(true, context.getString(R.string.msg_please_wait));
        if(isprocess)
            Utils.showProgressDialog(context,"");
        AppController.getInstance().getApiInterfaceTimeOut30Sec().getBillers(type,headers)
                    ...
                    public void onFailure(@NonNull Call<BaseResponse> call, @NonNull Throwable t) {
                        ...
                        if (t instanceof TimeoutException || t instanceof SocketTimeoutException) {
                            ...
                            if (retryCount < 3) {
                                ...
                                getbillerlist(context,headers,type,isprocess);
                            } else {
    ...
    public void getbillerdetails(Activity context,HashMap<String, String> headers,String type,Integer id,String name,boolean isprocess) {
        // getView().enableLoadingBar(true, context.getString(R.string.msg_please_wait));
        if(isprocess)
            Utils.showProgressDialog(context,"");
        AppController.getInstance().getApiInterfaceTimeOut30Sec().getBillersDetails(type,id,name,headers)
                .enqueue(new Callback<BaseResponse>() {
                    @Override
                    public void onResponse(@NonNull Call<BaseResponse> call, @NonNull Response<BaseResponse> response) {
                        // getView().enableLoadingBar(false, "");
                        ...
                        if (response.isSuccessful() && response.code() == 200)
                        {
                            getView().onBillerDetailsSuccess(response.body());
                        }else {
                            try {
                                error = response.errorBody().string();
                                jObjError = new JSONObject(error);
    ...
    public void getFetchbilllist(Activity context, HashMap<String, String> headers, RequestBody requestBody, boolean isprocess) {
        ...
        if(isprocess)
            Utils.showProgressDialog(context,"");
        AppController.getInstance().getApiInterfaceTimeOut30Sec().getsubscriptionfetchbill(headers,requestBody)
                .enqueue(new Callback<BaseResponse>() {
                    @Override
                    public void onResponse(@NonNull Call<BaseResponse> call, @NonNull Response<BaseResponse> response) {
                        // getView().enableLoadingBar(false, "");
    ...
    public void getPayBillList(Activity context,HashMap<String, String> headers, RequestBody requestBody,boolean isprocess) {
        ...
        if(isprocess)
            Utils.showProgressDialog(context,"");
        AppController.getInstance().getApiInterfaceTimeOut30Sec().getsubscriptionpaybill(headers,requestBody)
                .enqueue(new Callback<BaseResponse>() {
                    @Override
                    public void onResponse(@NonNull Call<BaseResponse> call, @NonNull Response<BaseResponse> response) {
                        // getView().enableLoadingBar(false, "");
