...
public class TwoFAVerifyPresenter extends BasePresenter<ITwoFAView>
{
    ...
    public void getBiometricVerify(Activity context,HashMap<String, String> headers, RequestBody requestBody,boolean isprocess) {
        ...
        if(isprocess)
            Utils.showProgressDialog(context,"");
        AppController.getInstance().getApiInterfaceTimeOut30Sec().verifyTwoFAFingurePrint(headers,requestBody)
                .enqueue(new Callback<BaseResponse>() {
                    @Override
                    public void onResponse(@NonNull Call<BaseResponse> call, @NonNull Response<BaseResponse> response) {
                        // getView().enableLoadingBar(false, "");
