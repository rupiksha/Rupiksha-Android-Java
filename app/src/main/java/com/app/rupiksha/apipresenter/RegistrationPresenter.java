...
public class RegistrationPresenter extends BasePresenter<IRegisterView>
{
    public static int retryCount = 0;

    public void getRole(Activity context, boolean isprocess) {
        if(isprocess)
            Utils.showProgressDialog(context,"");
        AppController.getInstance().getApiInterfaceTimeOut30Sec().getrole()
                .enqueue(new Callback<BaseResponse>() {
                    ...
                    public void onResponse(@NonNull Call<BaseResponse> call, @NonNull Response<BaseResponse> response) {
                        ...
                        {
                         getView().onRoleListSuccess(response.body());
                        }
                    }
    ...
    public void getRegistration(Activity context, RequestBody requestBody,boolean isprocess) {
        // getView().enableLoadingBar(true, context.getString(R.string.msg_please_wait));
        if(isprocess)
            Utils.showProgressDialog(context,"");
        AppController.getInstance().getApiInterfaceTimeOut30Sec().registration(requestBody)
                .enqueue(new Callback<BaseResponse>() {
                    @Override
                    public void onResponse(@NonNull Call<BaseResponse> call, @NonNull Response<BaseResponse> response) {
                        // getView().enableLoadingBar(false, "");
                    ...
                    public void onFailure(@NonNull Call<BaseResponse> call, @NonNull Throwable t) {
                        ...
                        if (t instanceof TimeoutException || t instanceof SocketTimeoutException) {
                            ...
                            if (retryCount < 3) {
                                getView().onErrorToast(context.getString(R.string.msg_internet_seems_slow));
                                getRegistration(context, requestBody,isprocess);
                            } else {
                                getView().onErrorToast(context.getString(R.string.msg_unable_connect_server));
                                //  getView().enableLoadingBar(false, "");
                                Utils.hideProgressDialog();
