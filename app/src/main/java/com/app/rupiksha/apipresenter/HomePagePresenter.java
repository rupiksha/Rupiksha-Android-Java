...
public class HomePagePresenter extends BasePresenter<IHomeView>
{
    ...
    public void getViewProfile(Activity context, HashMap<String, String> headers, boolean isprocess)
    {
        if(isprocess)
            Utils.showProgressDialog(context,"");
        AppController.getInstance().getApiInterfaceTimeOut30Sec().getUserInfo(headers)
                .enqueue(new Callback<BaseResponse>() {
                    ...
                    public void onResponse(@NonNull Call<BaseResponse> call, @NonNull Response<BaseResponse> response) {
                        ...
                        {
                            getView().onViewProfileSuccess(response.body());
                        }else {
                            try {
                    ...
                    public void onFailure(@NonNull Call<BaseResponse> call, @NonNull Throwable t) {
                        ...
                        if (t instanceof TimeoutException || t instanceof SocketTimeoutException) {
                            ...
                            if (retryCount < 3) {
                                ...
                                getViewProfile(context,headers,isprocess);
                            } else {
    ...
    public void getUserLogout(Activity context, HashMap<String, String> headers, boolean isprocess)
    {
        if(isprocess)
            Utils.showProgressDialog(context,"");
        AppController.getInstance().getApiInterfaceTimeOut30Sec().getUserLogout(headers)
                .enqueue(new Callback<BaseResponse>() {
                    @Override
                    public void onResponse(@NonNull Call<BaseResponse> call, @NonNull Response<BaseResponse> response) {
                        // getView().enableLoadingBar(false, "");
                        ...
                        if (response.isSuccessful() && response.code() == 200)
                        {
                            getView().onCheckLogoutSuccess(response.body());
                        }else {
                            try {
                                error = response.errorBody().string();
                                jObjError = new JSONObject(error);
                    ...
                    public void onFailure(@NonNull Call<BaseResponse> call, @NonNull Throwable t) {
                        ...
                        if (t instanceof TimeoutException || t instanceof SocketTimeoutException) {
                            ...
                            if (retryCount < 3) {
                                getView().onErrorToast(context.getString(R.string.msg_internet_seems_slow));
                                getUserLogout(context,headers,isprocess);
                            } else {
                                getView().onErrorToast(context.getString(R.string.msg_unable_connect_server));
                                //  getView().enableLoadingBar(false, "");
                                Utils.hideProgressDialog();
    ...
    public void getUserStatus(Activity context,HashMap<String, String> headers, RequestBody requestBody,boolean isprocess) {
        // getView().enableLoadingBar(true, context.getString(R.string.msg_please_wait));
        if(isprocess)
            Utils.showProgressDialog(context,"");
        AppController.getInstance().getApiInterfaceTimeOut30Sec().checkuserlocation(headers,requestBody)
                .enqueue(new Callback<BaseResponse>() {
                    @Override
                    public void onResponse(@NonNull Call<BaseResponse> call, @NonNull Response<BaseResponse> response) {
                        // getView().enableLoadingBar(false, "");
                        ...
                        if (response.isSuccessful() && response.code() == 200)
                        {
                            getView().onCheckStatusSuccess(response.body());
                        }else {
                            try {
                                error = response.errorBody().string();
                                jObjError = new JSONObject(error);
                    ...
                    public void onFailure(@NonNull Call<BaseResponse> call, @NonNull Throwable t) {
                        ...
                        if (t instanceof TimeoutException || t instanceof SocketTimeoutException) {
                            ...
                            if (retryCount < 3) {
                                getView().onErrorToast(context.getString(R.string.msg_internet_seems_slow));
                                getUserStatus(context,headers, requestBody,isprocess);
                            } else {
                                getView().onErrorToast(context.getString(R.string.msg_unable_connect_server));
                                //  getView().enableLoadingBar(false, "");
                                Utils.hideProgressDialog();
