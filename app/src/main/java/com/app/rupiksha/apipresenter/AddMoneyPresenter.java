...
public class AddMoneyPresenter extends BasePresenter<IAddMoneyView>
{
    ...
    public void getBankList(Activity context,HashMap<String, String> headers, boolean isprocess) {
        ...
        AppController.getInstance().getApiInterfaceTimeOut30Sec().getfundbanklist(headers)
    ...
    public void getAddMoney(Activity context, HashMap<String, String> headers, HashMap<String, RequestBody> map, boolean isprocess, MultipartBody.Part uploadedPan)
    {
        if(isprocess)
            Utils.showProgressDialog(context,"");
        AppController.getInstance().getApiInterfaceTimeOut30Sec().getAddMoney(headers,map,uploadedPan)
                .enqueue(new Callback<BaseResponse>() {
                    @Override
                    public void onResponse(@NonNull Call<BaseResponse> call, @NonNull Response<BaseResponse> response) {
                        // getView().enableLoadingBar(false, "");
                        ...
                        {
                            getView().onAddMoneySuccess(response.body());
                        }else {
                            try {
                                error = response.errorBody().string();
                                jObjError = new JSONObject(error);
                    ...
                    public void onFailure(@NonNull Call<BaseResponse> call, @NonNull Throwable t) {
                        ...
                        if (t instanceof TimeoutException || t instanceof SocketTimeoutException) {
                            ...
                            if (retryCount < 3) {
                                getView().onErrorToast(context.getString(R.string.msg_internet_seems_slow));
                                getAddMoney(context, headers,map,isprocess, uploadedPan);
                            } else {
                                getView().onErrorToast(context.getString(R.string.msg_unable_connect_server));
                                //  getView().enableLoadingBar(false, "");
                                Utils.hideProgressDialog();
