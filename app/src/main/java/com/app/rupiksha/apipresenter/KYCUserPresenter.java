...
public class KYCUserPresenter extends BasePresenter<IUserKycView>
{
    ...
    public void getDocumentList(Activity context, boolean isprocess) {
        if(isprocess)
            Utils.showProgressDialog(context,"");
        AppController.getInstance().getApiInterfaceTimeOut30Sec().getDocumentList()
                .enqueue(new Callback<BaseResponse>() {
                    @Override
                    public void onResponse(@NonNull Call<BaseResponse> call, @NonNull Response<BaseResponse> response) {
                        ...
                        {
                            getView().onDocumentListSuccess(response.body());
                        }
                    }
                    ...
                    public void onFailure(@NonNull Call<BaseResponse> call, @NonNull Throwable t) {
                        ...
                        if (t instanceof TimeoutException || t instanceof SocketTimeoutException) {
                            ...
                            if (retryCount < 3) {
                                getView().onErrorToast(context.getString(R.string.msg_internet_seems_slow));
                                getDocumentList(context, isprocess);
                            } else {
                                getView().onErrorToast(context.getString(R.string.msg_unable_connect_server));
                                //getView().enableLoadingBar(false, "");
                                Utils.hideProgressDialog();
    ...
    public void getKyc(Activity context, HashMap<String, String> headers, HashMap<String, RequestBody> map, boolean isprocess, MultipartBody.Part uploadedPan, MultipartBody.Part uploadedAadharFront, MultipartBody.Part uploadedAadharBack, MultipartBody.Part uploaddocument, MultipartBody.Part uploadselfie)
    {
        if(isprocess)
            Utils.showProgressDialog(context,"");
        AppController.getInstance().getApiInterfaceTimeOut30Sec().getuserKyc(headers,map,uploadedPan,uploadedAadharFront,uploadedAadharBack,uploaddocument,uploadselfie)
                .enqueue(new Callback<BaseResponse>() {
                    @Override
                    public void onResponse(@NonNull Call<BaseResponse> call, @NonNull Response<BaseResponse> response) {
                        // getView().enableLoadingBar(false, "");
                        ...
                        {
                            getView().onKycSuccess(response.body());
                        }else {
                            try {
                                error = response.errorBody().string();
                                jObjError = new JSONObject(error);
                    ...
                    public void onFailure(@NonNull Call<BaseResponse> call, @NonNull Throwable t) {
                        ...
                        if (t instanceof TimeoutException || t instanceof SocketTimeoutException) {
                            ...
                            if (retryCount < 3) {
                                getView().onErrorToast(context.getString(R.string.msg_internet_seems_slow));
                                getKyc(context, headers,map,isprocess, uploadedPan, uploadedAadharFront, uploadedAadharBack, uploaddocument, uploadselfie);
                            } else {
                                getView().onErrorToast(context.getString(R.string.msg_unable_connect_server));
                                //  getView().enableLoadingBar(false, "");
                                Utils.hideProgressDialog();
