...
import com.app.rupiksha.databinding.WalletReportLayoutBinding;
import com.app.rupiksha.models.AEPSReportDetailModel;

import java.util.List;

public class WalletReportDetailAdapter extends RecyclerView.Adapter<WalletReportDetailAdapter.ViewHolder> {
    private Context context;
    private List<AEPSReportDetailModel> list;
    WalletReportDetailAdapter.OnItemClick onItemClick;
    private int answerNum = -1;
    public WalletReportDetailAdapter(Context context, List<AEPSReportDetailModel> stepList, OnItemClick onItemClick) {
    ...
    public ViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        WalletReportLayoutBinding view = DataBindingUtil.bind(LayoutInflater.from(parent.getContext()).inflate(R.layout.wallet_report_layout, parent, false));
    ...
    public void onBindViewHolder(@NonNull ViewHolder holder, int position)
    {
        ...
        holder.binding.txType.setText(list.get(position).getTxntype());
        holder.binding.type.setText(list.get(position).getType());

        holder.binding.msg.setText(list.get(position).getRemark());


        if(list.get(position).getTxntype().equalsIgnoreCase("DEBIT")){
        ...
        String wallet1= String.valueOf(list.get(position).getOpening());
        holder.binding.opbal.setText(context.getResources().getString(R.string.rupees) + " " +wallet1);
        String wallet2= String.valueOf(list.get(position).getClosing());
        holder.binding.clb.setText(context.getResources().getString(R.string.rupees) + " " +wallet2);
    ...
    public class ViewHolder extends RecyclerView.ViewHolder {
        private WalletReportLayoutBinding binding;

        public ViewHolder(@NonNull WalletReportLayoutBinding itemView) {
